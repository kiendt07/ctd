...
void compileVarDecls(void) {
  ...
  if (lookAhead->tokenType == KW_VAR) {
    eat(KW_VAR);
    do {
      varObj = (Object **) malloc(sizeof(Object));
      objCount=0;
      do {
        eat(TK_IDENT);
        objCount++;
        varObj = (Object **)realloc(varObj, sizeof(Object ) * objCount);
        checkFreshIdent(currentToken->string);
        varObj[objCount-1] = createVariableObject(currentToken->string);
        if (lookAhead->tokenType == SB_COMMA) eat(SB_COMMA);
      } while (lookAhead->tokenType == TK_IDENT);
      eat(SB_COLON);
      varType = compileType();
      for (int i = 0; i<objCount; i++){
        varObj[i]->varAttrs->type = varType;
